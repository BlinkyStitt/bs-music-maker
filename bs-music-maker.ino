...
#define VS1053_RESET A0 // VS1053 reset pin (set to -1 to disable)
...
void setupMusicPlayer() {
  ...
  // TODO: for some reason when we first start, the system is a LOT quieter than when it wakes from sleep
  music_player.setVolume(0, 0);
}

void setup() {
  debug_serial(115200, 4000);
  ...
  // TODO: wtf. this is WAY quieter than when it wakes up from sleep. why?!
  music_player.sineTest(0x44, 300); // Make a tone for 300ms to indicate VS1053 is working
  delay(300);
  ...
  // TODO: well this is one way to fix the volume not being right until we sleep once...
  sleepUntilInterrupt();
}
  ...
  if (g_music_stopped) {
    ...
    if (digitalRead(START_PIN) == HIGH) {
      ...
      if (off_frames >= sleep_frames) {
        ...
        */
        sleepUntilInterrupt();  // START_PIN going HIGH will wake us up
